...
public final class EGOWeaponItems {
	 ...
	 * 创建远程武器构建器
	 */
	private static RemoteEgoWeaponBuilder onRemote() {
	 ...
	 * 创建近战武器构建器
	 */
	private static MeleeEgoWeaponBuilder onMelee() {
	...
	private static float minuteToSpeedConversion(float attackSpeed) {
		return (attackSpeed - 4);
	}

	private enum SpecialTemplateType implements TemplateType {
		/**
		 * 特殊武器
		 */
	...
	private enum RemoteTemplateType implements TemplateType {
		/**
		 * 加农炮
		 */
		CANNON("cannon", EGOEquipmentConstants.CANNON, 5, 15),
		/**
		 * 枪
		 */
		GUN("gun", EGOEquipmentConstants.GUN, -1, -1),
		/**
		 * 手枪
		 */
		PISTOL("pistol", EGOEquipmentConstants.PISTOL, 0.667f, 10),
		/**
		 * 来复枪
		 */
		RIFLE("rifle", EGOEquipmentConstants.RIFLE, 1, 15),
		/**
		 * 弩
		 */
		CROSSBOW("crossbow", EGOEquipmentConstants.CROSSBOW, 2, 20);
		private final String name;
		private final Set<DeferredItem<? extends Item>> set;
	...
	private enum MeleeTemplateType implements TemplateType {
		/**
		 * 斧
		 */
		AXE("axe", EGOEquipmentConstants.AXE, 1, 2),
		/**
		 * 拳套
		 */
		FIST("fist", EGOEquipmentConstants.FIST, 4, 2),
		/**
		 * 锤
		 */
		HAMMER("hammer", EGOEquipmentConstants.HAMMER, 1.15f, 5),
		/**
		 * 刀
		 */
		KNIFE("knife", EGOEquipmentConstants.KNIFE, 2.4f, 2),
		/**
		 * 梲
		 */
		MACE("mace", EGOEquipmentConstants.MACE, 1.6f, 3),
		/**
		 * 矛
		 */
		SPEAR("spear", EGOEquipmentConstants.SPEAR, 1.2f, 4),
		/**
		 * 剑
		 */
		SWORDS("swords", EGOEquipmentConstants.SWORDS, 1.6f, 0),
		;
		private final String name;
		private final Set<DeferredItem<? extends Item>> set;
		private final float attackSpeed;
	...
	private abstract static class EgoWeaponBuilder<I extends EgoWeaponBuilder<I>> {
		...
		public I model(ResourceLocation modelPath) {
	...
	private static class RemoteEgoWeaponBuilder extends EgoWeaponBuilder<RemoteEgoWeaponBuilder> {
		protected RemoteTemplateType templateType;
		protected RemoteEgoWeaponItem.Builder weaponBuilder = new RemoteEgoWeaponItem.Builder();
	...
	private static class MeleeEgoWeaponBuilder extends EgoWeaponBuilder<MeleeEgoWeaponBuilder> {
		protected MeleeTemplateType templateType;
		protected IMeleeEgoWeaponItem.Builder weaponBuilder = new IMeleeEgoWeaponItem.Builder();
