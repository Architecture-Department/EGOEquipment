...
public final class DatagenItemModel extends ItemModelProvider {
	...
	private void chaosSword(Item item) {
		...
		createModelFile(item, "weapon/", map, getParent("item/handheld"), LibItemPropertyRenderersRegistrar.CURRENT_LC_DAMAGE_TYPE);
	}
	...
	public ModelFile customModelFile(String name) {
		return new ModelFile.UncheckedModelFile(EGOEquipment.modRl(name));
	}
}
